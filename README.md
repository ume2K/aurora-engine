...
* **Database & Task Queue:** PostgreSQL. Dient als persistenter Speicher für User/Metadaten und agiert via `FOR UPDATE SKIP LOCKED` als robuste Message Queue für die verteilten Worker.
* **Storage:** RustFS (S3-kompatibler lokaler Objektspeicher für die reinen Mediendateien).
...
4. **Recovery:** Die überlebende Instanz erkennt den abgebrochenen Task in der PostgreSQL-Queue, übernimmt ihn ('Claiming') und beendet die Arbeit ohne Datenverlust für den Endnutzer.

## Status
* [x] Projektidee und Architektur-Entscheidung
* [ ] Infrastruktur Setup (`docker-compose.yml`)
* [ ] GoCore Integration & API Base
* [ ] ...
